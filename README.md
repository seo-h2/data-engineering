...
```sql
-- In Bigquery
SELECT count(1) FROM `my-project123-486104.zoomcamp.fct_monthly_zone_revenue`
```
...
SELECT pickup_zone
FROM (
    SELECT pickup_zone, row_number() over (order by sum(revenue_monthly_total_amount) desc) AS rn FROM `my-project123-486104.zoomcamp.fct_monthly_zone_revenue`
    WHERE service_type = 'Green' AND contains_substr(revenue_month, "2020")
...
FROM `my-project123-486104.zoomcamp.fct_monthly_zone_revenue`
WHERE service_type = 'Green' AND contains_substr(revenue_month, "2019-10")
...
```sql
SELECT count(1) FROM `my-project123-486104.zoomcamp.stg_fhv_tripdata` 
```